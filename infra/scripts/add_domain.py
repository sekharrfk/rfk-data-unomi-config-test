...
def add_kustomization(env, domain_hash):
    environment = jinja2.Environment(loader=FileSystemLoader("../../scripts/templates/"))
    template = environment.get_template("kustomization.j2")
    rendered_template = template.render(env=env, domain_hash=domain_hash)
    file_name = "../k8s/kustomize/overlays/{env}/{domain_hash}/kustomization.yaml".format(env=env, domain_hash=domain_hash)
...
def add_ingress(env, domain_hash):
    ...
    file_name = "../k8s/kustomize/overlays/{env}/ingress/ingress.yaml".format(env=env)
...
def add_argo_cd_application(env, domain_hash, branch):
    ...
    file_name = "../k8s/argocd/{env}/child-applications/{domain_hash}.yaml".format(env=env, domain_hash=domain_hash)
    os.makedirs(os.path.dirname("../k8s/argocd/{env}/child-applications".format(env=env)), exist_ok=True)
    write_to_file(file_name, rendered_template)
